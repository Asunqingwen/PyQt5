...
class Widget(QWidget):
    def __init__(self, parent=None):
        ...
        QTimer.singleShot(3000, self.giveHelp)  # 避免受窗口大小重绘事件的影响，可以将参数0变为3000，即3s，然后再运行
    ...
    def mouseMoveEvent(self, event):
        if not self.justDoubleclicked:
            ...
            self.text = '''鼠标位置：
            窗口坐标：QPoint({0},{1})
            屏幕坐标：QPoint({2},{3})'''.format(event.pos().x(), event.pos().y(), globalPos.x(), globalPos.y())
            self.update()
    ...
    '''重新实现其他事件，适用于PyQt没有提供该事件的处理函数的情况
        Tab键由于涉及到焦点切换，不会传递给keyPressEvent，因此需要在这里
        重新定义
    '''
